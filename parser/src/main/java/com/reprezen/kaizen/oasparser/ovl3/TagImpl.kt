...
class TagImpl : PropertiesOverlay<Tag> ,Tag {
	...
	companion object {
		...
		val factory = object : OverlayFactory<Tag>() {
			...
			override fun getOverlayClass() : Class<out JsonOverlay<in Tag>> {
				return TagImpl::class.java
			}
		
			override fun _create(tag : Tag?, parent : JsonOverlay<*>?, refMgr : ReferenceManager) : JsonOverlay<Tag> {
				return TagImpl(tag, parent, refMgr)
			}
		
			override fun _create(json : JsonNode, parent : JsonOverlay<*>?, refMgr : ReferenceManager) : JsonOverlay<Tag> {
		...
		private fun getSubtypeOf(tag : Tag) : Class<out Tag> {
			return Tag::class.java
		}

		private fun getSubtypeOf(json : JsonNode) : Class<out Tag> {
			return Tag::class.java
		}
